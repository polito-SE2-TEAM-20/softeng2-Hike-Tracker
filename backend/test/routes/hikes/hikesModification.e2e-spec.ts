    ...
    it('should return parking lots and huts in a certain radius', async () => {
        ...
        await restService
            ...
            .expect(({ body }) => {
                ...
                expect(mapToId(body.parkingLots)).toEqual([parkingLot2.id, parkingLot1.id]);
            });
        ...
        .expect(({ body }) => {
            ...
            expect(mapToId(body.huts)).toEqual([hut4.id, hut1.id]);
            expect(mapToId(body.parkingLots)).toEqual([parkingLot3.id, parkingLot2.id, parkingLot1.id]);
        });
    })
});
